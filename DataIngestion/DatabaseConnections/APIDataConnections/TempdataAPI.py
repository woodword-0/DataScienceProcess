...
file1 = "Query1.xlsx"
df_sales = pd.read_excel(file1)
df_sales.columns
# Customer Postal Codes and tokens
file3 = "Customer_Token_PostalCodes.xlsx"
# Sales data
file4 = "04192023Report.xlsx"
df_sales_new = pd.read_excel(path + file4)
df_sales_new.SalesDate.max()
# Customer Postal Codes and tokens 
...
# Concatenate the two dataframes on top of each other
df_concat = pd.concat([df_sales, df_sales_new], axis=0)
# Sort the concatenated dataframe by the 'SalesDate' column
df_concat = df_concat.sort_values('SalesDate')
# bug: this method creates a duplicate row
# It is currently dropped later in the script but will be edited in the future
#####################################################################################################################################################################
#####################################################################################################################################################################
# Sales DataFrame and customer tokens
df = pd.merge(df_concat, df_token, on='CustomerName', how='left')
#####################################################################################################################################################################
#####################################################################################################################################################################
...
# Fill NaN values in columns A, B, and C with the mean value of each column
full_df.fillna({'TAVG': full_df['TAVG'].mean(), 'TMIN':  full_df['TAVG'].mean(), 'TMAX': full_df['TAVG'].mean()}, inplace=True)
full_df.isna().sum()

full_df.shape

# drop duplicates
duplicates = full_df.loc[full_df.duplicated()]
duplicates
full_df = full_df.drop_duplicates()
full_df = full_df.asfreq('D')
full_df.isna().sum()
#####################################################################################################################################################################
#####################################################################################################################################################################
# Find Optimal lag
...
df
df.isna().sum()
with open("TrainData_06012022to04012023.pkl", "wb") as f:
    pd.to_pickle(df, f)  
